...
> It is also referenced by `../implement/references/html-to-holon-component-map.md`.
...
The six Acme CRM mockup HTML files map to **three** Java view classes.
Desktop and mobile variants of the same concern are implemented in **one** responsive
class — no separate `*Mobile*` classes are ever created.

| Mockup file | Emitted view class | Notes |
|---|---|---|
| `customer-list.html` | `<Entity>ListView` | Desktop list — `ListingBundle` + `ChipGroup` + KPI `Highlight` row |
| `customer-list.mobile.html` | `<Entity>ListView` *(same class)* | Mobile list rendered via `.mobileViewColumn(LitRendererBuilder.<T>gridCell()...)` on the same `ListingBundle`; FAB floated via CSS |
| `customer-detail.html` | `<Entity>MasterDetailView` | Desktop master-detail — `MasterDetailLayout` split view |
| `customer-detail.mobile.html` | `<Entity>MasterDetailView` *(same class)* | Mobile detail opened as bottom `Sheet` via `MasterDetailBuilder…viewMode(ViewMode.MOBILE).withMobileSheet(Sheet.Side.BOTTOM)` (builder method on `MasterDetailConfigurator`) |
| `customer-new.html` | `New<Entity>View` | Desktop new-entity form — `EntityCreationForm` + `FormStepCard` scaffold |
| `customer-new.mobile.html` | `New<Entity>View` *(same class)* | Mobile new form — same `EntityCreationForm`; `ResponsiveDiv` + `@media` CSS collapses to single column; `StickyActionBar` sticks to viewport bottom |

> **One-view-per-concern rule**: three mockup pairs → three view classes.
> `MasterDetailBuilder…viewMode(ViewMode.MOBILE).withMobileSheet(Sheet.Side.BOTTOM)` (a `MasterDetailConfigurator` builder method) handles the desktop/mobile
> split automatically. `EntityCreationForm` + `@media` CSS handles the form's responsive
> collapse. Emitting six view classes (one per mockup file) is incorrect.
...
| HTML region | Holon component | Correct API |
|---|---|---|
| Master list panel (left 340 px) + fluid detail panel | `MasterDetailLayout` | `MasterDetailBuilder.create(Customer.class).master(m->m.listing(svc::findSlice).columns("customerNumber","name","openAr","statusId").search(...)).detail(d->d.content(c->buildDetail(c))).withUrlSync(c->String.valueOf(c.getId()),id->svc.findById(Long.valueOf(id))).build()` — import `com.iyensoft.vaadin.flow.components.builders.MasterDetailBuilder` |
...
| Active contacts grid (2-col avatar cards) | `ListingBundle` + `Components.avatar()` | `Components.listing(Contact.class).columns("name","title","email").fetch(…).build()` — avatar via `Components.avatar(initials).name(fullName).colorIndex(i).build()` |
...
| Region / Industry / Country / Source dropdowns (creatable) | `Input.singleSelect(Long.class).allowCustomValues(true).onCustomValueSet(…)` | bound via `.bind(...)` on `EntityFormPanel` |
...
| Mobile detail (hero + quick-action bar + card stack) | `MasterDetailLayout` (builder) with `.withMobileSheet(Sheet.Side.BOTTOM)` | Wire via `MasterDetailBuilder.create(T.class).viewMode(ViewMode.MOBILE).withMobileSheet(Sheet.Side.BOTTOM)` — `withMobileSheet` is on `MasterDetailConfigurator`, NOT on `MasterDetailLayout` itself; master fills full screen on mobile |
...
> **Two namespaces** — do not mix them up:
> - **`com.iyensoft.vaadin.flow.components(.builders)`** — master-detail layout scaffolding (`MasterDetailLayout`, `MasterDetailAccent`, `Panel`, `DetailSyncAware`, and all builders).
> - **`com.holonplatform.vaadin.flow.vaadinplus.components`** — presentational widgets (`HeroStrip`, `ArAgingBar`, `EntityFormPanel`, `Tag`, `StatusBadge`, `ChipGroup`, `Sheet`, etc.).
> - **`com.holonplatform.vaadin.flow.components`** — core Holon: `ListingBundle`, `Components.listing(T.class)`, etc.
> Source branch: `vaithu/holon-vaadin-flow@modernize/java-20260526093038` (commit `1f8e38128571f8f6cdfcca6c68b2b935baff3fc7`).

| Component | Fully-qualified class | Builder / entry point | Source path in `vaithu/holon-vaadin-flow@modernize/java-20260526093038` |
|---|---|---|---|
| `MasterDetailLayout<T>` | `com.iyensoft.vaadin.flow.components.MasterDetailLayout` | `com.iyensoft.vaadin.flow.components.builders.MasterDetailBuilder.create(Class<T>)` or `MasterDetailBuilder.create(PropertySet<?>)`; `Components.masterDetail(T.class)` is a Javadoc-referenced alias (not independently verified) | `core/src/main/java/com/iyensoft/vaadin/flow/components/MasterDetailLayout.java` |
| `MasterDetailBuilder<T>` | `com.iyensoft.vaadin.flow.components.builders.MasterDetailBuilder` | `MasterDetailBuilder.create(Class<T>)` | `core/src/main/java/com/iyensoft/vaadin/flow/components/builders/MasterDetailBuilder.java` |
| `MasterDetailConfigurator<T,C>` | `com.iyensoft.vaadin.flow.components.builders.MasterDetailConfigurator` | `configure(MasterDetailLayout<T>, Class<T>)`; methods incl. `withMobileSheet(Sheet sheet)`, `withMobileSheet(Sheet.Side side)`, `viewMode(ViewMode)`, `withAccentColorProvider(Function<T,String>)`, `withUrlSync(...)` | `core/src/main/java/com/iyensoft/vaadin/flow/components/builders/MasterDetailConfigurator.java` |
| `MasterDetailAccent` (enum) | `com.iyensoft.vaadin.flow.components.MasterDetailAccent` | `.cssClass()` accessor (e.g. `MasterDetailAccent.SUCCESS.cssClass()`) | `core/src/main/java/com/iyensoft/vaadin/flow/components/MasterDetailAccent.java` |
| `LazyTabsBuilder` / `LazyTabsConfigurator` | `com.iyensoft.vaadin.flow.components.builders.LazyTabsBuilder` | `LazyTabsBuilder.create()` | `core/src/main/java/com/iyensoft/vaadin/flow/components/builders/LazyTabsBuilder.java` |
| `AppShellLayoutBuilder` | `com.iyensoft.vaadin.flow.components.builders.AppShellLayoutBuilder` | `AppShellLayoutBuilder.create()` | `core/src/main/java/com/iyensoft/vaadin/flow/components/builders/AppShellLayoutBuilder.java` |
| `HeroStrip` | `com.holonplatform.vaadin.flow.vaadinplus.components.HeroStrip` | `Components.heroStrip()` (Javadoc-confirmed); nested record types `HeroStrip.Header(Component thumbIcon, String ribbon, String name, boolean starred, String meta)`, `HeroStrip.Tag(String text, TagVariant)`, `HeroStrip.Cell(header, content, footer, boolean pulse, ValueVariant)`; enums `Variant` (DEFAULT/INFO/SUCCESS/WARNING/DANGER/VIOLET), `TagVariant` (OK/PRI/PRIM/VIOLET/WARN/HOT), `ValueVariant` (DEFAULT/OK/ALERT) | `core/src/main/java/com/holonplatform/vaadin/flow/vaadinplus/components/HeroStrip.java` |
| `ArAgingBar` | `com.holonplatform.vaadin.flow.vaadinplus.components.ArAgingBar` | `ArAgingBarBuilder.create()` — fluent `.title().icon().variant().segment(key,value,percent,Variant).leftStat().centerStat().rightStat().build()`; record-based `Segment(key, value, percent, Variant)`; `Variant` (DEFAULT/SUCCESS/WARNING/DANGER/INFO) | `core/src/main/java/com/holonplatform/vaadin/flow/vaadinplus/components/ArAgingBar.java` |
| `ArAgingBarBuilder` | `com.holonplatform.vaadin.flow.vaadinplus.components.ArAgingBar` (inner or companion) | `ArAgingBarBuilder.create()` | `core/src/main/java/com/holonplatform/vaadin/flow/vaadinplus/components/ArAgingBar.java` |
| `EntityFormPanel` | `com.holonplatform.vaadin.flow.vaadinplus.components.EntityFormPanel` | `EntityFormPanel.bean(T.class)` | `core/src/main/java/com/holonplatform/vaadin/flow/vaadinplus/components/EntityFormPanel.java` |
| `EntityCreationForm` | `com.holonplatform.vaadin.flow.vaadinplus.components.EntityCreationForm` | `Components.entityCreationForm()` | `core/src/main/java/com/holonplatform/vaadin/flow/vaadinplus/components/EntityCreationForm.java` |
| `FormStepCard` | `com.holonplatform.vaadin.flow.vaadinplus.components.FormStepCard` | `Components.formStepCard()` | `core/src/main/java/com/holonplatform/vaadin/flow/vaadinplus/components/FormStepCard.java` |
| `TimelineStepper` | `com.holonplatform.vaadin.flow.vaadinplus.components.TimelineStepper` | `Components.timelineStepper()` | `core/src/main/java/com/holonplatform/vaadin/flow/vaadinplus/components/TimelineStepper.java` |
| `StickyActionBar` | `com.holonplatform.vaadin.flow.vaadinplus.components.StickyActionBar` | `Components.stickyActionBar()` | `core/src/main/java/com/holonplatform/vaadin/flow/vaadinplus/components/StickyActionBar.java` |
| `LivePreviewCard` | `com.holonplatform.vaadin.flow.vaadinplus.components.LivePreviewCard` | `Components.livePreviewCard(Localizable)` | `core/src/main/java/com/holonplatform/vaadin/flow/vaadinplus/components/LivePreviewCard.java` |
| `ChecklistPanel` | `com.holonplatform.vaadin.flow.vaadinplus.components.ChecklistPanel` | `Components.checklistPanel()` | `core/src/main/java/com/holonplatform/vaadin/flow/vaadinplus/components/ChecklistPanel.java` |
| `StatusBadge` | `com.holonplatform.vaadin.flow.vaadinplus.components.StatusBadge` | `Components.statusBadge(Localizable, StatusBadge.Variant)` | `core/src/main/java/com/holonplatform/vaadin/flow/vaadinplus/components/StatusBadge.java` |
| `Tag` | `com.holonplatform.vaadin.flow.vaadinplus.components.Tag` | `new Tag(Localizable)` | `core/src/main/java/com/holonplatform/vaadin/flow/vaadinplus/components/Tag.java` |
| `ChipGroup` / `Chip` | `com.holonplatform.vaadin.flow.vaadinplus.components.ChipGroup` | `ChipGroup.create()` | `core/src/main/java/com/holonplatform/vaadin/flow/vaadinplus/components/ChipGroup.java` |
| `Sheet` / `Sheet.Side` | `com.holonplatform.vaadin.flow.vaadinplus.components.Sheet` | `Components.sheet(Sheet.Side)` | `core/src/main/java/com/holonplatform/vaadin/flow/vaadinplus/components/Sheet.java` |
| `ListingBundle<T>` | `com.holonplatform.vaadin.flow.components.ListingBundle` | `Components.listing(T.class)` | `core/src/main/java/com/holonplatform/vaadin/flow/components/ListingBundle.java` |
...
7. **MasterDetailLayout** handles both desktop split and mobile sheet automatically — wire mobile via `MasterDetailBuilder.create(T.class).viewMode(ViewMode.MOBILE).withMobileSheet(Sheet.Side.BOTTOM)` (the `withMobileSheet` method is on `MasterDetailConfigurator`, not on `MasterDetailLayout`); do not build two separate views.
